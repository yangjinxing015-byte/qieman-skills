#!/usr/bin/env python3
"""Upload only qieman-vip-advisor-visual-design into yanzichloe/skills.

Target path: qieman-skills/marketing-design/qieman-vip-advisor-visual-design/
Does not replace other files under qieman-skills/.
"""
...
REPO = "skills"
BRANCH = "main"
PREFIX = "qieman-skills/marketing-design/qieman-vip-advisor-visual-design"
BINARY_SUFFIXES = {".png", ".jpg", ".jpeg", ".gif", ".pdf", ".ico", ".webp", ".zip"}
CACHE_FILE = ".publish-vip-advisor-blob-cache.json"
BLOB_DELAY_SEC = 0.35
MAX_RETRIES = 5
...
def api(
    ...
    headers = {
        ...
        "User-Agent": "qieman-vip-advisor-publish",
    }
    body = None
    if data is not None:
...
def main() -> None:
    ...
    src = Path(__file__).resolve().parent.parent / "marketing-design" / "qieman-vip-advisor-visual-design"
    if not (src / "SKILL.md").is_file():
        sys.exit(f"错误: 找不到 skill 目录 {src}")
    ...
    new_commit = api(
        ...
        token,
        {
            "message": "Add qieman-vip-advisor-visual-design under marketing-design.",
            "tree": new_tree["sha"],
            "parents": [base_sha],
        },
    )
